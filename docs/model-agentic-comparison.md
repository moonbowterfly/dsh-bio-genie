...
| 卡死段 delta 到达速率从 50.1 ms/delta 恶化至 116.3 ms/delta（2.3 倍） | **上游生成速度持续递减**——支持「供应商负载升高（模型火爆→减速）」假说；但单次观测无对照组，「模型长序列生成自然减速/网络抖动」同样是候选解释 |
| 最后一个 delta 后 1.7s 出现 assistant/message+step/end+turn/end，turn/end reason=aborted(user) | 流始终健康传输，**无 HTTP 错误/429/5xx/重试**；是用户侧取消终止，非供应商「调用失败」 |
| step 9 reasoning 自述 "whole genome is huge" 却仍将序列整串写入 `sequence` 参数 | **模型决策层硬伤**，与供应商无关；对照 v4-flash 选择「2.1Mb 不能整串传参→bio_python 落盘」 |

> 彻底闭合「减速是否因供应商负载」归因需隔离实验：将 mimo-v2.5 挂另一
> provider 入口跑同任务同工具调用，对比 delta 到达速率与完成情况。
> 正常闭合/更快 → 供应商负载或转换问题；仍慢/仍不闭合 → 模型特性。

结论：maxTokens 是真实配置差异，但本次实测中**未触及、未构成瓶颈**；
主导因素为模型 agentic 生成质量（tool-call JSON 生成与终止能力、认知监控、
工具边界理解）。供应商侧「减速」有时序证据支持（速率递减 2.3 倍，符合
模型火爆→负载升高假说），但「调用失败」无证据（流健康、无错误、为用户
取消终止）；供应商问题需隔离实验定论。
