...
1. **选路径**：先查语义化工具表（39 个高频工具，快、省 token、参数有校验）→ 命中就用；没命中再用 `bio_python` 执行器写 Biopython/Python 代码。
...
| 层 | 工具 | 何时用 |
|---|---|---|
| 语义化工具 ×39 | `bio_seq_analyze` / `bio_enrichr` / `bio_deseq2` / `bio_gsea` / `bio_fig_export` 等 | 高频稳定操作，**第一优先** |
| Python 执行器 | `bio_python`（Python：Biopython 全功能 + 出版级绘图） | 语义化工具覆盖不到的一切，**第二优先** |
| 元工具 ×3 | `bio_env` / `bio_log` / `bio_memory` | 环境诊断、日志回溯、经验查询 |

合计 43 个工具。完整参数与返回结构 → 加载 `dsh-bio-genie-guide-tools`。
...
| 指南 skill | 内容 | 何时加载 |
|---|---|---|
| `dsh-bio-genie-guide-tools` | 43 个工具完整参数/返回/示例 | 不确定工具怎么用、参数怎么传时 |
