...
1. **选路径**：先查语义化工具表（44 个高频工具，快、省 token、参数有校验）→ 命中就用；没命中再用 `bio_python` 执行器写 Biopython/Python 代码。
...
合计 48 个工具。完整参数与返回结构 → 加载 `dsh-bio-genie-guide-tools`。
...
| 指南 skill | 内容 | 何时加载 |
|---|---|---|
| `dsh-bio-genie-guide-tools` | 48 个工具完整参数/返回/示例 | 不确定工具怎么用、参数怎么传时 |
