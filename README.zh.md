...
- **全功能覆盖**：`bio_python` 执行器可运行任意 Biopython 代码（比对、PDB、Phylo、motif、BLAST…），另有 14 个领域 skill 配方
...
重启 dsh web 服务后生效。首次启动时插件在后台自动引导 Python 环境
...
agent 自动：`bio_seq_io_read` → `bio_seq_analyze` → `bio_seq_restriction` → 汇总报告 + 生物学解读

**用户**：*「画一下这两个基因的蛋白结构比对」*（无对应语义化工具）

agent 自动：加载 `bio-align` / `bio-structure` skill → `bio_python` 写 Biopython 程序执行 → 产出文件 + 报告

## 🔧 环境引导

首次调用（或 dsh 启动后台预热）时插件：

1. 下载 uv（GitHub release；`DSH_BIO_UV_BASE` 可换镜像加速）
