...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    /**
     * 代谢建模分页（域插件 dsh-bio-gem 的状态面板）。
     *
     * ⚠️ 本组件**只在 gem 已安装时被挂载** —— BioGenieSection 依据
     * /api/dsh-bio-genie/metabolic 返回的 installed 字段决定是否给出该 tab，
     * 未安装时该分页完全不出现（不渲染空壳页）。数据由宿主侧端点读取 gem 的
     * 运行时目录，本组件不发起跨插件请求。
     */
    function MetabolicView(props) {
      var d = props.data || {}
      var models = (d.models && d.models.items) || []
      var ledger = d.ledger || { ledgers: [] }
      var py = d.python || {}
      ...
      return createElement('div', null,
        ...
        createElement(Card, { title: '域插件：dsh-bio-gem（代谢建模）' },
          createElement('table', { style: styles.kvTable },
            createElement('tbody', null,
              kvRow('状态', createElement(Status, { kind: 'ok' }, '已安装 · 与本插件共存')),
              kvRow('版本', d.version || '—'),
              kvRow('探测方式', d.detectedBy || '—'),
              kvRow('插件目录', createElement('span', { style: { wordBreak: 'break-all' } }, d.pluginDir || '—')),
              kvRow('数据根', createElement('span', { style: { wordBreak: 'break-all' } }, d.dataRoot || '—')),
            ),
          ),
          createElement('p', { style: styles.note },
            '本分页由 BioGenie 面板托管（dsh-bio-gem 自身不注册设置入口）；',
            '代谢建模能力以 ' + ((d.tools || []).length) + ' 个 gem_* 工具形式提供给 agent。'),
        ),

        createElement(Card, { title: 'Python 环境（gem 工具的解释器）' },
          sel
            ? createElement('table', { style: styles.kvTable },
                createElement('tbody', null,
          ...
          (py.candidates || []).length
            ...
            : null,
          createElement('p', { style: styles.note },
            '顺序与 dsh-bio-gem 的选择逻辑一致：GEM_PYTHON → 宿主自举环境 → CONDA_PREFIX → PATH；',
            '逐个探测 import cobra，选中第一个可用者。'),
        ),

        createElement(Card, { title: '模型（' + ((d.models && d.models.count) || 0) + '）' },
          models.length === 0
            ? createElement(Status, { kind: 'warn' }, '暂无模型文件 —— 构建后用 gem_build 产出，存放于数据根的 models/ 目录')
            : createElement('table', { style: styles.pkgTable },
                createElement('thead', null,
                  createElement('tr', null,
        ...
        createElement(Card, { title: '预测账本（' + (ledger.count || 0) + ' 个模型 / ' + (ledger.totalEntries || 0) + ' 条）' },
          (ledger.ledgers || []).length === 0
            ? createElement(Status, { kind: 'warn' }, '暂无账本条目 —— 必需性/分泌谱/双敲除等预测会自动登记（一个模型一个账本）')
            : createElement('table', { style: styles.pkgTable },
                createElement('thead', null,
                  createElement('tr', null,
          ...
          createElement('p', { style: styles.note }, '账本目录：', ledger.dir || '—'),
        ),
        ...
        createElement(Card, { title: '构建引擎（gem_build）' },
          createElement('table', { style: styles.kvTable },
            createElement('tbody', null,
              ...
              kvRow('gapseq（WSL2，质量档）',
                createElement(Status, { kind: 'warn' }, '面板不做昂贵探测')),
            ),
          ),
          ...
          createElement('p', { style: styles.note },
            '构建侧依赖不与本插件的自举环境共享（CarveMe 需独立 venv + diamond 二进制；gapseq 需 WSL2）。'),
        ),
        ...
        createElement(Card, { title: '工具清单（' + ((d.tools || []).length) + '）' },
          createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, lineHeight: 1.9 } },
            (d.tools || []).map(function (t) {
              return createElement('code', { key: t, style: { fontSize: '0.82em', opacity: 0.8 } }, t)
            }),
          ),
          createElement('p', { style: styles.note },
            '这些工具与 bio_* 工具注册在同一实例，agent 按系统提示的「代谢模型能力域」路由段自动选择。'),
        ),
      )
    }
    ...
    function BioGenieSection() {
      ...
      // 域插件探测（挂载时拉一次）：决定「代谢建模」分页是否出现。
      // 失败或未安装时静默降级为「无该分页」，不向用户暴露错误。
      var gemState = useState({ data: null })
      var gem = gemState[0], setGem = gemState[1]
      useState(function () {
        rpc('/metabolic').then(function (r) {
          if (r.ok) setGem({ data: r.value })
        })
        return null
      })
      ...
      return createElement('div', { className: 'biogenie-settings', style: styles.section },
        ...
        createElement('p', { className: 'biogenie-settings-lead', style: styles.lead },
          ...
          '53 个工具（48 语义化 + 执行器/元工具）、47 个 skill、零依赖自举 Python 环境。',
        ),
        ...
        tab === 'overview' ? createElement(OverviewTab, null)
                  ...
                  : (tab === 'metabolic' && gemInstalled) ? createElement(MetabolicView, { data: gem.data })
                  : null,
      )
    }
