...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var DOCS = [
      ...
      { label: '工具表', href: META.docsRoot + '/tools.md', note: '36 个语义化工具的字段与适用场景' },
      { label: 'skill 导航', href: META.docsRoot + '/skills.md', note: '21 领域/研究 + 17 协议 + 8 指南 skill 的领域与语言标注' },
    ...
    function AddonsView() {
      ...
      if (s.status === 'loading') {
        return createElement(Card, { title: '高级功能模块' },
          createElement(Status, { kind: 'loading' }, '检测模块安装状态……（import probe）'),
        )
      }
      if (s.status === 'err') {
        return createElement(Card, { title: '高级功能模块' },
    ...
    /**
     * 工作区视图：展示/设置 bio agent 默认工作区（A 路径）。
     * 设置后，bio-genie 预设会话未显式指定工作区时自动以 A 为默认工作区；
     * 清空则恢复 dsh 默认（插件保底 ~/.dsh/sessions/default）。
     */
    function WorkspaceView() {
      var state = useState({ status: 'loading', data: null, error: null, saved: false })
      var s = state[0], set = state[1]
      var pathState = useState('')
      var path = pathState[0], setPath = pathState[1]

      function reload() {
        set({ status: 'loading', data: null, error: null, saved: false })
        rpc('/workspace-config').then(function (r) {
          if (r.ok) {
            var v = r.value || {}
            set({ status: 'ok', data: v, error: null, saved: false })
            setPath(v.defaultWorkspace || '')
          } else {
            set({ status: 'err', data: null, error: r, saved: false })
          }
        })
      }
      ...
      function save() {
        var val = path.trim()
        set({ status: 'ok', data: s.data, error: null, saved: true })
        rpcPost('/workspace-config', { defaultWorkspace: val || null }).then(function (r) {
          if (r.ok) { reload(); setPath(val) }
          else set({ status: 'err', data: s.data, error: r, saved: false })
        })
      }

      if (s.status === 'loading') {
        return createElement(Card, { title: '默认工作区' },
          createElement(Status, { kind: 'loading' }, '加载中……'))
      }
      if (s.status === 'err') {
        return createElement(Card, { title: '默认工作区' },
      ...
      var configured = s.data && s.data.configured

      return createElement(Card, { title: '默认工作区（bio agent A 路径）' },
        createElement('p', { style: styles.note },
          '设置后：使用 bio-genie 预设创建会话且「未指定工作区」时，自动以该路径为默认工作区；',
          '不填 / 清空 = 保持 dsh 默认（插件保底 ~/.dsh/sessions/default，每任务独立子目录）。',
        ),
        createElement('div', { style: { display: 'flex', gap: 8, alignItems: 'center', marginTop: 8 } },
          createElement('input', {
            type: 'text',
            value: path,
            placeholder: '例如 D:/Program/dsh/my-workspace',
            onChange: function (e) { setPath(e.target.value) },
            style: { flex: 1, padding: '6px 10px', font: 'inherit', color: 'inherit', background: 'transparent', border: '1px solid currentColor', borderRadius: 4 },
          }),
          createElement('button', {
            onClick: save,
            style: { padding: '6px 12px', font: 'inherit', cursor: 'pointer', border: '1px solid currentColor', borderRadius: 4, background: 'transparent', color: 'inherit' },
          }, '保存'),
        ),
        createElement('p', { style: styles.note },
          configured
            ? '当前已启用默认工作区：' + String(s.data.defaultWorkspace)
            : '当前未设置（dsh 默认行为）',
          s.saved ? ' · 已保存' : '',
        ),
        createElement('p', { style: styles.note },
                  '请填绝对路径（如 D:/Program/dsh/my-workspace），保存时会校验。',
                  '路径不存在时将自动创建；若本机不可用（盘符缺失/无权限）则自动回退默认工作区（~/.dsh/sessions/default）。',
                  '清空后保存可清除设置。')
              )
    }
     ...
     * 内部维护 tab 状态：'overview' | 'skills' | 'debug' | 'python' | 'addons'。
     */
    function BioGenieSection() {
      ...
      return createElement('div', { className: 'biogenie-settings', style: styles.section },
        ...
        createElement('p', { className: 'biogenie-settings-lead', style: styles.lead },
          ...
          '44 个语义化工具、47 个 skill、零依赖自举 Python 环境。',
        ),
        ...
        tab === 'overview' ? createElement(OverviewTab, null)
                  ...
                  : tab === 'workspace' ? createElement(WorkspaceView, null)
                  : null,
      )
    }
