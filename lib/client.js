...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    /** R 环境设置卡片：显示 persistentR/warmUpR 状态 */
    function RSettingsCard() {
      var persistentR = DEFAULT_CONFIG.find(function(c) { return c.key === 'persistentR' })
      var warmUpR = DEFAULT_CONFIG.find(function(c) { return c.key === 'warmUpR' })
          ...
          createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
            ...
            createElement('span', {
              style: {
                ...
                background: persistentR && persistentR.value === 'true' ? '#2d8a4e' : '#666',
                color: '#fff', fontSize: '0.82em', fontWeight: 600,
              }
            }, persistentR && persistentR.value === 'true' ? '开启' : '关闭'),
            createElement('span', { style: { fontSize: '0.78em', opacity: 0.5 } },
              persistentR ? persistentR.desc : ''),
          ),
          // warmUpR 开关
          createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
            ...
            createElement('span', {
              style: {
                display: 'inline-block', padding: '3px 10px', borderRadius: 4,
                background: warmUpR && warmUpR.value === 'true' ? '#2d8a4e' : '#666',
                color: '#fff', fontSize: '0.82em', fontWeight: 600,
              }
            }, warmUpR && warmUpR.value === 'true' ? '开启' : '关闭'),
            createElement('span', { style: { fontSize: '0.78em', opacity: 0.5 } },
              warmUpR ? warmUpR.desc : ''),
          ),
          // 说明
          createElement('p', { style: { fontSize: '0.82em', opacity: 0.5, marginTop: 8 } },
            '修改方式：编辑 cordis.patch.yml → plugins.dsh-bio-genie.persistentR / warmUpR → 重启 dsh。',
          ),
        ),
      )
    }
